...
export async function GET(
  ...
  try {
    ...
    const avg_fitness = generations.map((g: any) => g.avg_fitness).filter(Boolean) as number[]
    const peak_fitness = generations.map((g: any) => g.peak_fitness).filter(Boolean) as number[]
    ...
    const analysis = {
      ...
      final_avg_fitness: avg_fitness[avg_fitness.length - 1] || null,
      final_peak_fitness: peak_fitness[peak_fitness.length - 1] || null,
      peak_fitness_overall: peak_fitness.length > 0 ? Math.max(...peak_fitness) : null,
